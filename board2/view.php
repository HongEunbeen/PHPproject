<?php
    ...
    $is_html      = $row[is_html];
    $is_file_name_0 = $row[file_name_0];
    $is_file_name_1 = $row[file_name_0];
    $is_file_name_2 = $row[file_name_0];
    $is_file_copied_0 = $row[file_copied_0];
    $is_file_copied_1 = $row[file_copied_0];
    $is_file_copied_2 = $row[file_copied_0];
    ...
    $sql = "update concert set hit=$new_hit where num=$num";//조회수가 증가된 값을 넣어준다
	...
	<div id = "wrap">
		<div id = "content">
			<div id = "col2">
				...
				<div class = "clear"></div>
				<form method="post" action="insert.php?c_mode=insert&num=<?=$num?>">
					<input type = "hidden" value = "<?=$usernick?>" name = "nickname">
					<textarea  row = "15" cols = "79" name = "c_content"></textarea>
					<input type = "image" src = "../img/ripple.png">
				</form> 
				<?php 
				echo "item nick $usernick";
    				$sql = "select * from c_comment where g_num=$num order by c_num desc";
    				$result = mysql_query($sql,$connect);
